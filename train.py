...
def multi_task_trainer(train_loader, test_loader, multi_task_model, device, optimizer, opt, logger, start_ep=0, total_it=0):
    ...
    for ep in range(start_ep, total_epoch): ## 每一个epoch 计算一次动态权重
        ...
        for it, (img_ir, img_vi, label, bi, bd, mask) in enumerate(train_loader):
            ...
            mask = mask.to(device)
            seg_pred, fused_img = multi_task_model(img_vi, img_ir)            
            ...
            fusion_loss, int_loss, grad_loss = Fusion_loss(vi_Y, img_ir, fused_img, mask=mask, device=device)            
        
            train_loss = 20 * train_seg_loss + fusion_loss
            train_loss.backward()
            optimizer.step()
        ...
        logger.info('ep: [{}/{}], learning rate: {:.6f}, time consuming: {:.2f}s, segmentation loss: {:.4f}, fusion loss: {:.4f}'.format(ep+1, total_epoch, lr, training_time, seg_loss.item(), fusion_loss.item()))
        logger.info('grad loss: [{:.4f}], int loss: [{:.4f}], segmentation loss: {:.4f}, mIou: {:.4f}, Acc: {:.4f}, Eta: {}\n'.format(grad_loss.item(), int_loss.item(), seg_loss.item(), mIoU, Acc, eta))
        start = time()
