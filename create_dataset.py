...
class MSRSData(torch.utils.data.Dataset):
    ...
    def __init__(self, opts, is_train=True, crop=lambda x: x):
        ...
        if is_train:
            self.vis_folder = os.path.join(opts.dataroot,'MSRS','train', 'vi')
            self.ir_folder = os.path.join(opts.dataroot, 'MSRS','train', 'ir')
            self.label_folder = os.path.join(opts.dataroot,'MSRS','train', 'label')
            self.bi_folder = os.path.join(opts.dataroot,'MSRS','train', 'bi')
            self.bd_folder = os.path.join(opts.dataroot,'MSRS','train', 'bd')
            self.mask_folder = os.path.join(opts.dataroot,'MSRS','train', 'mask')
        else:
            self.vis_folder = os.path.join(opts.dataroot,'MSRS','test', 'vi')
            self.ir_folder = os.path.join(opts.dataroot,'MSRS','test', 'ir')
            self.label_folder = os.path.join(opts.dataroot,'MSRS','test', 'label')
